...
FCF_DECLARE_FUNCTION(test_sig, "*", test_dsig, double(*) (double),
    double test_dsig(double a_value){
      return (double)1.0 / (1.0 + exp(-a_value));
    })

FCF_DECLARE_FUNCTION(test_sig, "*", test_fsig, float(*) (float),
...
void declareFunctionsTest() {
  ...
  unsigned int findex = FCF_GET_FUNCTION_INDEX("test_sig", double(double));
  double (*testFunc)(double) = (double(*)(double))FCF_GET_FUNCTION(findex);
